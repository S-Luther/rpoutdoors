                ...
                snapshot.forEach((doc) => {
                  ...
                  JSON.parse(localStorage.getItem("log").slice(0, -1)+"]}")["orders"].forEach((item) => {
                    if(item["id"] === data.id){
                      ...
                      firebase
                      .firestore()
                      .collection('Apparell').doc(data.id).update({
                        "inventory.large" :  data.inventory["large"] - islarge,
                        "inventory.medium" :  data.inventory["medium"] - ismedium,
                        "inventory.small" :  data.inventory["small"] - issmall,
                        "inventory.x-large" :  data.inventory["x-large"] - isxlarge,
                        "inventory.x-small" :  data.inventory["x-small"] - isxsmall,
                        "inventory.xx-large" :  data.inventory["xx-large"] - isxxlarge,
                      });
                })
...
JSON.parse(localStorage.getItem("log").slice(0, -1)+"]}")["orders"].forEach((item) => {
  ...
  if(!(item["size"] === undefined) && !(item["color"] === "undefined") ){
    console.log(item["size"])
    orderSummary = orderSummary+' in '+item["size"]+'\n'
  }
  if(!(item["orientation"] === undefined) && !(item["orientation"] === "undefined")){
